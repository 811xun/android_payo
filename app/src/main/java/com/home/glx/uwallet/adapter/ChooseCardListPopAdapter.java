...
public class ChooseCardListPopAdapter extends RecyclerView.Adapter {
    ...
    public ChooseCardListPopAdapter(Context context, List<BankDatas> listData, String selectId) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        switch (viewType) {
            case 123:
                return new AddBottom(inflater.inflate(R.layout.item_addbank, parent, false));
            case 143:
                return new ItemView(inflater.inflate(R.layout.item_choose_bank, parent, false));
        }
        return null;
    }
    ...
    class ItemView extends RecyclerView.ViewHolder {
        ...
        public void binData(final int position, final BankDatas bankDatas) {
            //切换字体
            TypefaceUtil.replaceFont(idBankName, "fonts/gilroy_medium.ttf");
...
//            Glide.with(context).load("http://paytest-api.loancloud.cn/app/getBankLogo?bankName=" + bankDatas.getBankName() + "&type=1").into(idItemBankImg);
            ...
            if (bankDatas.getCustomerCcType() != null) {
                if (bankDatas.getCustomerCcType().equals("10")) {
                    idItemBankImg.setImageResource(R.mipmap.pay_visa_logo_new);
                } else if (bankDatas.getCustomerCcType().equals("20")) {
                    idItemBankImg.setImageResource(R.mipmap.pay_master_logo_new);
                } else if (bankDatas.getCustomerCcType().equals("60")){
                    idItemBankImg.setImageResource(R.mipmap.pay_onther_logo_new);
                }else {
            ...
            if (mposition == -1) {
                if (!TextUtils.isEmpty(selectId)) {
                    if (bankDatas.getId().equals(selectId)) {
                        itemRl.setBackground(context.getDrawable(R.mipmap.bg_orange));
                        bankDatas.setChoice(true);
                    } else {
                        itemRl.setBackground(context.getDrawable(R.mipmap.bg));
...
//                        notifyDataSetChanged();
    ...
    class AddBottom extends RecyclerView.ViewHolder {
        ...
        public void binData() {
            //切换字体
            TypefaceUtil.replaceFont(idAddText, "fonts/gilroy_medium.ttf");
