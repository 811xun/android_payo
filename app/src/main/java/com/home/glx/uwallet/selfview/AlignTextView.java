            ...
            for (int i = 0; i < linCount; i++) {
                ...
                switch (mAlign) {
                    ...
                    default:
                        ...
                        if (linCount > 1 && line.toString().contains("\n")) { //最后一行保持默认方式
                            canvas.drawText(line, 0, line.length(), layout.getParagraphLeft(i) + getPaddingLeft(), layout.getLineBottom(i), paint);
                        }else if ((linCount > 1 && i == linCount - 1) || linCount == 1) {
                            canvas.drawText(line, 0, line.length(), layout.getParagraphLeft(i) + getPaddingLeft(), layout.getLineBottom(i) + 12, paint);
