 ...
 */
public class PwdEditText_Fenkai extends AppCompatEditText {//输入框下面画短的横线
    ...
    private int Circle = 16, Round;//Circle:直径都是4
    private boolean isPwd, isWaitInput;

    public PwdEditText_Fenkai(Context context) {
        super(context);
    ...
    public PwdEditText_Fenkai(Context context, AttributeSet attrs) {
        super(context, attrs);
        mC = context;
        setAttrs(attrs);
        init();
    ...
    public PwdEditText_Fenkai(Context context, AttributeSet attrs, int defStyleAttr) {
...
//            Circle = t.getDimensionPixelSize(R.styleable.PwdEditText_circle, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 4, getResources().getDisplayMetrics()));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        Paint borderPaint;//边界画笔
        int borderColor = ContextCompat.getColor(getContext(), R.color.huise);
        int borderWidth = 5;
        ...
        for (int i = 0; i < textLength; i++) {
            ...
            RectF rect = new RectF(i * Wide + spzceX + 10, spzceY, i * Wide + Wide - spzceX - 10, Wide - spzceY); //四个值，分别代表4条线，距离起点位置的线
...
//            canvas.drawRoundRect(rect, Round, Round, sidePaint); //绘制边框；
        ...
        for (int j = 0; j < mText.length(); j++) {
            if (isPwd) {
                canvas.drawCircle(rectFS.get(j).centerX(), rectFS.get(j).centerY(), dip2px(mC,5), textPaint);
            } else {
